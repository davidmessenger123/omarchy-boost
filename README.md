...
An Omarchy bar widget that lets you cap your CPU's max boost clock on the fly, with optional thermal protection, temporary MAX boosts, and live CPU telemetry.
...
- **Thermal Guard** watches the package temperature and applies a lower cap at the trigger threshold. It uses a separate release threshold and cooldown, and keeps the latest manual cap as the restoration target.
- **Temporary boost** raises the cap to MAX for a configurable countdown, then restores the latest manual cap. An active Thermal Guard always takes precedence.
- **Live graphs** show sampled frequency, package power, and aggregate CPU utilization. Power is shown as unavailable when the kernel exposes RAPL only to root.
...
boot by the `cpu-cap-boost.service` systemd monitor. The root helper owns the
manual baseline, Thermal Guard latch, and temporary boost deadline in
validated, root-owned state.
...
helper (to write `scaling_max_freq`) and a continuously running monitor
service. The installer verifies one opened snapshot of the helper against a
pinned SHA-256 digest before writing it, then writes a narrowly scoped polkit
rule and the service, deriving the target user automatically. Re-run it after
upgrading so the new policy engine and telemetry service are installed:
...
| `boostctl.py` | Unprivileged read-only fallback monitor for sysfs, sensors, frequency, and utilization |
| `boostset.py` | Root-owned policy engine: applies caps, manages Thermal Guard/countdowns, and provides the fixed user-action helper |
| `boostmonitord.py` | Root-owned systemd-only copy of the policy engine that runs the monitor and samples telemetry |
| `persist.py` | Validated, transactionally locked writer for the widget's shell.json entry |
| `setup.py` | Installs/removes the root helper, polkit rule, monitor service, and state |
| `tests/test_boost.py` | Standard-library tests for privilege boundaries, validation, persistence, policy transitions, telemetry, and cleanup primitives |
| `/var/lib/omarchy-boost/maxboost` | Root-owned compatibility record of the latest manual baseline |
| `/var/lib/omarchy-boost/maxboost.policy` | Root-owned guard/countdown policy and latched state |
| `/run/omarchy-boost/state.json` | Read-only live telemetry and effective policy snapshot |
...
`/sys/devices/system/cpu/cpu*/cpufreq/scaling_max_freq` controls and fixed
root-owned policy state under `/var/lib/omarchy-boost`. It rejects extra
arguments, non-finite/out-of-range values, symlinked or foreign state files,
and serializes sysfs application with policy persistence under one lock. It
snapshots every target's prior cap and restores those values if any write,
readback, or persistence step fails. The root monitor samples temperature,
frequency, utilization, and deduplicated top-level RAPL package zones, then
publishes a sanitized snapshot at `/run/omarchy-boost/state.json`. The widget
elevates policy actions through `pkexec`; the unprivileged fallback monitor is
never authorized by the polkit rule.
...
reported full-turbo, and `boostset.py` enforces the same ceiling per core when
it writes `scaling_max_freq` — so neither the widget nor a policy action can
ever exceed what the CPU reports.
...
## Thermal Guard, countdown, and live graphs

Thermal Guard is opt-in. Set `thermalHighC` to the package temperature that
should trigger protection, `thermalLowC` to a lower release temperature,
`thermalCapGHz` to the protected cap, and `thermalCooldown` to the number of
seconds the package must stay cool before the manual cap is restored. A missing
temperature sensor never releases an existing latch. The popup contains the
same settings, and the root monitor keeps the latch active while the shell is
restarting.

The temporary boost button uses `boostMinutes` (1–1440), raises the effective
cap to MAX (or the configured `capMax` ceiling), and restores the current manual
baseline at expiry. Manual slider changes cancel the countdown. Thermal Guard
has higher priority than a countdown, so a hot CPU cannot be forced to MAX by
the timer.

The live panel keeps a short in-memory history of current frequency, package
power, and aggregate utilization. RAPL power is read by the root monitor;
unsupported or permission-restricted zones are shown as `N/A` rather than
...
1. The separate action helper at
   `/usr/local/libexec/omarchy-boost/boostset.py`, its systemd-only monitor copy
   at `/usr/local/libexec/omarchy-boost/boostmonitord.py`, and the private state
...
3. The continuously running policy monitor:
   ...
   [Service]
   Type=simple
   ExecStart=/usr/bin/python3 -I /usr/local/libexec/omarchy-boost/boostmonitord.py run
   Restart=on-failure
   RestartSec=2
   ...
   It is enabled and started with `systemctl enable --now`. `--uninstall`
   disables/removes the service, rule, helper, policy state, runtime state,
   lock file, and state directory without following symlinks. The polkit rule
   never authorizes the user-writable plugin directory. The only root-writable
   application paths are fixed sysfs controls, fixed policy state, and the
...
- `/var/lib/omarchy-boost/maxboost` holds the latest successful manual baseline;
  `/var/lib/omarchy-boost/maxboost.policy` holds the root-owned guard and
  countdown state. An invalid policy fails closed instead of choosing a cap.
- The monitor clears expired countdowns and reboots with a safe policy; a
  missing temperature sensor cannot release a latched Thermal Guard.
